...
Vector3 PathTracerMIS::Evaluate(Ray ray, int depth) {
    ...
    if (material->emission != 0.) {
        ...
        BxDFProbability *= light->DirectionalSamplePDF(intersection.intersect, BxDFDirection);
        Vector3 BxDFColor;
        if (BxDFProbability != 0.)  {
            ...
            if (intersector.getIntersect({intersection.intersect, BxDFDirection}, lightIntersection)) {
                if (lightIntersection.primitive != light) {
                    ...
                    BxDFColor = lightIntersection.primitive->material->color * lightSampleBxDF;
                }
            }
        }
        // Weight probabilities with power heuristic
        return intersector.scene->lights.size() * (lightColor * PowerHeuristic(lightProbability, BxDFProbability) + BxDFColor * PowerHeuristic(BxDFProbability, lightProbability)) + BxDFColor * Evaluate({intersection.intersect, BxDFDirection}, depth+1);
    }
}
